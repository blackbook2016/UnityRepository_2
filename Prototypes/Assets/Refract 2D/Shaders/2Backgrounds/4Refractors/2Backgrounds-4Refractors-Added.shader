Shader "Refract2D/2 Backgrounds/4 Refractors/Added" {
	...
	Properties {
		...
		_DistortionMap4 ("Distortion Map", 2D) = "" {}	//Receive input from a Texture on Texture Unit 3
	...
	SubShader {
		...
		Pass {
			...
			uniform sampler2D _DistortionMap4;				//Define _DistortionMap4 from Texture Unit 3 to be sampled in 2D
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed2 distortion = tex2D(_DistortionMap, float2(_DistortionScaleX,_DistortionScaleY)*i.uv+float2(_DistortionScrollX,_DistortionScrollY).xy) * _DistortionPower - (_DistortionPower*0.5);	//Get distortion map, Red=X, Green=Y
 				...
 				fixed2 distortion4 = tex2D(_DistortionMap4, float2(_DistortionScaleX4,_DistortionScaleY4)*i.uv+float2(_DistortionScrollX4,_DistortionScrollY4).xy) * _DistortionPower4 - (_DistortionPower4*0.5);	//Get distortion map, Red=X, Green=Y
  				fixed distort=distortion+distortion2+distortion3+distortion4;	//For speed
  				fixed4 background = tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+(_Refraction*distort)+float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background
   				...
   				return fixed4((background.r+background2.r)*_BackgroundFade2,(background.g+background2.g)*_BackgroundFade2,(background.b+background2.b)*_BackgroundFade2,1.0);	//Return added backgrounds
			}
