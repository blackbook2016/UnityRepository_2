Shader "Refract2D/2 Backgrounds/1 Refractor/1 Midground/Multiplied" {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed2 distortion = tex2D(_DistortionMap, float2(_DistortionScaleX,_DistortionScaleY)*i.uv+float2(_DistortionScrollX,_DistortionScrollY).xy) * _DistortionPower - (_DistortionPower*0.5);	//Get distortion map, Red=X, Green=Y
   				...
   				return fixed4(midground.r*background2.r*_BackgroundFade2,midground.g*background2.g*_BackgroundFade2,midground.b*background2.b*_BackgroundFade2,1.0);	//Alpha blend backgrounds together
			}
