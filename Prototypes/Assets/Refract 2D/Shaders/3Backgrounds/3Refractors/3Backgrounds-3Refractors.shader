Shader "Refract2D/3 Backgrounds/3 Refractors" {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion3 = tex2D(_DistortionMap3, float2(_DistortionScaleX3,_DistortionScaleY3)*i.uv+float2(_DistortionScrollX3,_DistortionScrollY3).xy) * _DistortionPower3 - (_DistortionPower3*0.5);	//Get distortion map, Red=X, Green=Y
   				...
   				return fixed4(background3*_BackgroundFade3)+((1.0-_BackgroundFade3)*fixed4((background.r*alpha)+(background2.r*inverse),(background.g*alpha)+(background2.g*inverse),(background.b*alpha)+(background2.b*inverse),1.0));	//Return alphablended backgrounds
			}
