Shader "Refract2D/2 Backgrounds/2 Refractors/1 Midground/Multiplied" {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion2 = tex2D(_DistortionMap2, float2(_DistortionScaleX2,_DistortionScaleY2)*i.uv+float2(_DistortionScrollX2,_DistortionScrollY2).xy) * _DistortionPower2 - (_DistortionPower2*0.5);	//Get distortion map, Red=X, Green=Y
   				...
   				fixed4 background = tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+(_Refraction*(distortion+distortion2))+float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background
   				fixed4 background2 = tex2D(_Background2, float2(_BackgroundScaleX2,_BackgroundScaleY2)*i.uv+(_Refraction2*(distortion+distortion2))+float2(_BackgroundScrollX2,_BackgroundScrollY2));	//Get the background
   				...
   				return fixed4(midground.r*background2.r*_BackgroundFade2,midground.g*background2.g*_BackgroundFade2,midground.b*background2.b*_BackgroundFade2,1.0);	//Multiply backgrounds together
			}
