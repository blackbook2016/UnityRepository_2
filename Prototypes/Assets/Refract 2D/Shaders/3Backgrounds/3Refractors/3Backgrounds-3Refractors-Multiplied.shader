Shader "Refract2D/3 Backgrounds/3 Refractors/Multiplied" {
	...
	SubShader {
		...
		Pass {
			...
			#pragma target 3.0				//Run this shader on at least Shader Model 2.0 hardware (e.g. Direct3D 9)
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed2 distortion = tex2D(_DistortionMap, float2(_DistortionScaleX,_DistortionScaleY)*i.uv+float2(_DistortionScrollX,_DistortionScrollY).xy) * _DistortionPower - (_DistortionPower*0.5);	//Get distortion map, Red=X, Green=Y
 				...
 				fixed distort=distortion+distortion2+distortion3;	//For speed
  				...
  				return fixed4(background3*_BackgroundFade3)*fixed4(background.r*background2.r*_BackgroundFade2,background.g*background2.g*_BackgroundFade2,background.b*background2.b*_BackgroundFade2,1.0);	//Return multiplied backgrounds
			}
