Shader "Refract2D/3 Backgrounds/2 Refractors/1 Midground/Multiplied" {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion2 = tex2D(_DistortionMap2, float2(_DistortionScaleX2,_DistortionScaleY2)*i.uv+float2(_DistortionScrollX2,_DistortionScrollY2).xy) * _DistortionPower2 - (_DistortionPower2*0.5);	//Get distortion map, Red=X, Green=Y
  				...
  				fixed alpha=midground.a;
   				...
   				midground = fixed4((midground.r*alpha)+(background.r*inverse),(midground.g*alpha)+(background.g*inverse),(midground.b*alpha)+(background.b*inverse),1.0);	//Return alpha blended layers
   				return fixed4(background3*_BackgroundFade3)*fixed4(midground.r*background2.r*_BackgroundFade2,midground.g*background2.g*_BackgroundFade2,midground.b*background2.b*_BackgroundFade2,1.0);	//Alpha blend backgrounds together
			}
