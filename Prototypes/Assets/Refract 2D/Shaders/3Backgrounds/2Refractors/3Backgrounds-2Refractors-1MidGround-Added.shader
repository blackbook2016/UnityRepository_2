Shader "Refract2D/3 Backgrounds/2 Refractors/1 Midground/Added" {
	...
	Properties {
		...
		_DistortionMap2 ("Distortion Map", 2D) = "" {}	//Receive input from a Texture on Texture Unit 4
		...
		_Midground ("Midground", 2D) = "" {}			//Receive input from a Texture on Texture Unit 5
	...
	SubShader {
		...
		Pass {
			...
			uniform sampler2D _DistortionMap2;				//Define _DistortionMap2 from Texture Unit 4 to be sampled in 2D
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion2 = tex2D(_DistortionMap2, float2(_DistortionScaleX2,_DistortionScaleY2)*i.uv+float2(_DistortionScrollX2,_DistortionScrollY2).xy) * _DistortionPower2 - (_DistortionPower2*0.5);	//Get distortion map, Red=X, Green=Y
   				...
   				fixed distort=distortion+distortion2;	//For speed
   				...
   				fixed4 background3 = tex2D(_Background3, float2(_BackgroundScaleX3,_BackgroundScaleY3)*i.uv+(_Refraction2*distort)+float2(_BackgroundScrollX3,_BackgroundScrollY3));	//Get the background
   				fixed alpha=midground.a;
   				fixed inverse=1.0-alpha;
   				midground = fixed4((midground.r*alpha)+(background.r*inverse),(midground.g*alpha)+(background.g*inverse),(midground.b*alpha)+(background.b*inverse),1.0);	//Alpha blended layers
   				return fixed4(background3*_BackgroundFade3)+fixed4((midground.r+background2.r)*_BackgroundFade2,(midground.g+background2.g)*_BackgroundFade2,(midground.b+background2.b)*_BackgroundFade2,1.0);	//Added backgrounds together
			}
