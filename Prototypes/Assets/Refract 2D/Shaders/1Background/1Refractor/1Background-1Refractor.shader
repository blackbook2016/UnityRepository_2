Shader "Refract2D/1 Background/1 Refractor" {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				////fixed2 distortion = tex2D(_DistortionMap, float2(_DistortionScaleX,_DistortionScaleY)*i.uv+float2(_DistortionScrollX,_DistortionScrollY).xy) * _DistortionPower - (_DistortionPower*0.5);	//Get distortion map, Red=X, Green=Y
   				////return fixed4(tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+distortion+float2(_BackgroundScrollX,_BackgroundScrollY)));	//Get the background
   				return tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+(-_Refraction*(tex2D(_DistortionMap, float2(_DistortionScaleX,_DistortionScaleY)*i.uv+float2(_DistortionScrollX,_DistortionScrollY).xy) * _DistortionPower - (_DistortionPower*0.5)))+float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background and distort
			}
