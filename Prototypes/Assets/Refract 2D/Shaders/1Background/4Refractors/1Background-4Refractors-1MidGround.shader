Shader "Refract2D/1 Background/4 Refractors/1 Midground" {
	...
	Properties {
		...
		_DistortionMap2 ("Distortion Map", 2D) = "" {}	//Receive input from a Texture on Texture Unit 2
		...
		_DistortionMap4 ("Distortion Map", 2D) = "" {}	//Receive input from a Texture on Texture Unit 4
		_DistortionScrollX4 ("X Offset", float) = 0		//Receive input from float
		_DistortionScrollY4 ("Y Offset", float) = 0		//Receive input from float
		_DistortionScaleX4 ("X Scale", float) = 1.0		//Receive input from float
		_DistortionScaleY4 ("Y Scale", float) = 1.0		//Receive input from float
		_DistortionPower4("Distortion Power", float) = 0.08	//Receive input from float
	...
	SubShader {
		...
		Pass {
			...
			uniform sampler2D _DistortionMap4;				//Define _DistortionMap4 from Texture Unit 4 to be sampled in 2D
			//uniform float4 _DistortionMap4_ST;			//Use the Float _DistortionMap4_ST to pass the Offset and Tiling for the texture(s)
			uniform sampler2D _Midground;					//Define _Midground from Texture Unit 5 to be sampled in 2D
			...
			uniform float _DistortionScrollX4;
			uniform float _DistortionScrollY4; 
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion4 = tex2D(_DistortionMap4, float2(_DistortionScaleX4,_DistortionScaleY4)*i.uv+float2(_DistortionScrollX4,_DistortionScrollY4).xy) * _DistortionPower4 - (_DistortionPower4*0.5);	//Get distortion map, Red=X, Green=Y
  				fixed4 midground = tex2D(_Midground, float2(_MidgroundScaleX,_MidgroundScaleY)*i.uv+float2(_MidgroundScrollX,_MidgroundScrollY));	//Get the midground
   				...
   				fixed4 background = tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+(_Refraction*(distortion+distortion2+distortion3+distortion4))+float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background
