Shader "Refract2D/1 Background/3 Refractors/1 Midground" {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				fixed2 distortion3 = tex2D(_DistortionMap3, float2(_DistortionScaleX3,_DistortionScaleY3)*i.uv+float2(_DistortionScrollX3,_DistortionScrollY3).xy) * _DistortionPower3 - (_DistortionPower3*0.5);	//Get distortion map, Red=X, Green=Y
  				fixed4 midground = tex2D(_Midground, float2(_MidgroundScaleX,_MidgroundScaleY)*i.uv+float2(_MidgroundScrollX,_MidgroundScrollY));	//Get the midground
   				...
   				fixed4 background = tex2D(_Background, float2(_BackgroundScaleX,_BackgroundScaleY)*i.uv+(_Refraction*(distortion+distortion2+distortion3))+float2(_BackgroundScrollX,_BackgroundScrollY));	//Get the background
