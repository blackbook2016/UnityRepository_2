Shader "Refract2D/3 Backgrounds/3 Refractors/Added" {
	...
	Properties {
		...
		_DistortionMap3 ("Distortion Map", 2D) = "" {}	//Receive input from a Texture on Texture Unit 5
	...
	SubShader {
		...
		Pass {
			...
			uniform sampler2D _DistortionMap3;				//Define _DistortionMap3 from Texture Unit 5 to be sampled in 2D
			//uniform float4 _DistortionMap2_ST;			//Use the Float _DistortionMap3_ST to pass the Offset and Tiling for the texture(s)
				...
				fixed2 distortion2 = tex2D(_DistortionMap2, float2(_DistortionScaleX2,_DistortionScaleY2)*i.uv+float2(_DistortionScrollX2,_DistortionScrollY2).xy) * _DistortionPower2 - (_DistortionPower2*0.5);	//Get distortion map, Red=X, Green=Y
 				fixed2 distortion3 = tex2D(_DistortionMap3, float2(_DistortionScaleX3,_DistortionScaleY3)*i.uv+float2(_DistortionScrollX3,_DistortionScrollY3).xy) * _DistortionPower3 - (_DistortionPower3*0.5);	//Get distortion map, Red=X, Green=Y
  				fixed distort=distortion+distortion2+distortion3;	//For speed
   				...
   				return fixed4(background3*_BackgroundFade3)+fixed4((background.r+background2.r)*_BackgroundFade2,(background.g+background2.g)*_BackgroundFade2,(background.b+background2.b)*_BackgroundFade2,1.0);	//Return added backgrounds
			}
